...
export default function JerarquiaEditPage() {
  // SELECTOR UDN
  ...
  const dataJerarquia = getJerarquia(params.id);

  useEffect(() => {
    if (dataJerarquia.status) {
      setDataForm({
        idJerarquia: dataJerarquia.data[0].id_fila,
        nivel: dataJerarquia.data[0].nivel,
        detalle: dataJerarquia.data[0].detalle,
        udn: dataJerarquia.data[0].udn,
      });
    }
  }, [dataJerarquia]);

  if (!dataJerarquia.status) return <PageLoader />;
  return (
    <>
      {/* <PageLoader /> */}
                              ...
                              {dataUDN.status &&
                                dataUDN.data.map((i) => (
                                  <option
                                    value={`${i.descripcion}`}
                                    selected={
                                      dataForm.udn == i.descripcion ||
                                      dataForm.udn == i.id_uc
                                        ? true
                                        : false
                                    }
                                  >
